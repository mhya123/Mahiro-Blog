...
> [!ai] ChatGPT-5.4-Mini
> 二开从改功能前先重构目录与组件边界，目标是降低后续改动风险。建议按页面层、业务组件层、工具层三层拆分，页面层保持轻薄，优先抽离导航状态、卡片列表、筛选搜索、配置读取等高复用且高变动模块。
> 重构时每次只做一个动作：先搬运不改逻辑，再微调结构，最后清理冗余，并持续执行检查与构建防回归。应避免巨型组件、页面直写业务逻辑、工具函数依赖 UI 状态，节奏上先拆导航/卡片/列表，再处理配置与数据映射，最后整理动画与交互层。
